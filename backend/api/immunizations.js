...
import { withBoosterStatus } from "./_lib/schedule.js";
  ...
  try {
    if (req.method === "GET") {
      ...
      const transformed = Array.isArray(data)
        ? data.map(withBoosterStatus)
        : withBoosterStatus(data);

      if (dueOnly && Array.isArray(transformed)) {
        return res.status(200).json(transformed.filter((item) => item.is_booster_due));
      }
    ...
    if (req.method === "POST") {
      ...
      if (error) throw error;
      return res.status(201).json(withBoosterStatus(data));
    }
    ...
    if (req.method === "PATCH") {
      ...
      if (error) throw error;
      return res.status(200).json(withBoosterStatus(data));
    }
